...
func (as *AuthService) CheckForExistingUser(field, value string) (*model.User, error) {
	return as.Repository.CheckForExistingUser(field, value)
}

func (as *AuthService) Signup(signupData model.SignupInput) (*model.User, *model.Token, *model.AuthUserProfile, error) {

	if signupData.MobileNo == "" && signupData.Email == "" {
		return nil, nil, nil, fmt.Errorf("either email or mobile number is required")
	}
	if signupData.MobileNo != "" {
		...
		if mobileResult != nil {
			return nil, nil, nil, fmt.Errorf("user with given mobile number %v already exist", signupData.MobileNo)
		}
	}
	...
	if signupData.Email != "" {
		...
		if emailResult != nil {
			return nil, nil, nil, fmt.Errorf("user with given email address %v already exist", signupData.Email)
		}
	}
...
func (as *AuthService) ValidateToken(authHeader string) (*model.User, error) {
...
func (as *AuthService) RefreshToken(refreshTokenStr string) (*model.User, *model.Token, *model.AuthUserProfile, *model.AuthUserMembership, error) {
    ...
    refreshToken, err := as.Repository.FindRefreshTokenByIdentifier(claims.TokenId)
    ...
    if refreshToken.ExpiresAt.Before(time.Now()) {
        ...
        if claims.User == nil || claims.User.ID == uuid.Nil {
            ...
            user = u
			token, err := as.Repository.CreateToken(user, config.ClientId(), config.ClientSecret(), nil)
			if err != nil {
				return nil, nil, nil, nil, ErrInvalidToken
			}
			// Fetch user profile
...
func (as *AuthService) FetchUser(userID uuid.UUID) (*model.User, error) {
	return as.Repository.FetchUser(userID)
}
